% Multiclass Support vector machine (10/12/2020)

function MSVM = mMultiClassSupportVectorMachineECOC(feat,label,opts)
% Default
fun   = 'r'; 
kfold = 10;
tf    = 2;
...
if tf == 1
  fprintf('\n Accuracy (MSVM-HO): %g %%',100 * acc);
elseif tf == 2
  fprintf('\n Accuracy (MSVM-CV): %g %%',100 * acc);
elseif tf == 3
  fprintf('\n Accuracy (MSVM-LOO): %g %%',100 * acc);
end  
end
